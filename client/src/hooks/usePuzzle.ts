  ...
  const loadPuzzle = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "Не удалось загрузить задачу");
      }
      ...
      setStatus("error");
      setError(loadError instanceof Error ? loadError.message : "Не удалось загрузить задачу");
    }
  }, [clearResetTimeout, clearSystemMoveTimeout, puzzleId]);
