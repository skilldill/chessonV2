    ...
    async function pickPuzzle() {
      ...
      try {
        ...
        if (!response.ok || !data.success || !data.puzzle?.id) {
          throw new Error(data.error || "Не удалось подобрать задачу");
        }
        ...
        if (!ignore) {
          setError(loadError instanceof Error ? loadError.message : "Не удалось подобрать задачу");
        }
      }
    }
  ...
  return (
    <main className="min-h-screen bg-[#10141f] px-4 py-8 text-white">
      <div className="mx-auto flex min-h-[70vh] w-full max-w-xl flex-col items-center justify-center gap-5 text-center">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-normal">Подбираем задачу</h1>
          <p className="mt-2 text-sm text-white/60">Ищем подходящую тактическую позицию</p>
        </div>
