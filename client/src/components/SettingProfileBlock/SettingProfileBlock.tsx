import { useState, type FC } from "react";
import { MemAvatarSelect } from "../MemAvatarSelect/MemAvatarSelect";

export const SettingProfileBlock: FC = () => {
    const [name, setName] = useState("");
    const [showAvatarSelect, setShowAvatarSelect] = useState(false);
    const [showNameInput, setShowNameInput] = useState(true);

    const handleNext = () => {
        if (name.trim()) {
            setShowNameInput(false);
            // Wait for fade-out animation to complete, then show avatar select
            setTimeout(() => {
                setShowAvatarSelect(true);
            }, 700);
        }
    };

    const handleSelectAvatar = (index: number) => {
        console.log("Selected avatar:", index, "with name:", name);
        // Handle avatar selection and name here
    };

    const handleBack = () => {
        setShowAvatarSelect(false);
        // Wait for fade-out animation to complete, then show name input
        setTimeout(() => {
            setShowNameInput(true);
        }, 700);
    };

    return (
        <div className="w-[472px] h-[452px] flex flex-col items-center py-12 gap-12 relative overflow-hidden">
            {/* Name Input Step */}
            <div 
                className={`absolute inset-0 flex flex-col items-center gap-12 py-12 transition-all duration-700 ease-in-out ${
                    showNameInput 
                        ? 'opacity-100 translate-x-0 scale-100' 
                        : 'opacity-0 -translate-x-[100px] scale-95 pointer-events-none'
                }`}
            >
                <h3 
                    className="text-3xl font-semibold transition-all duration-700"
                    style={{
                        opacity: showNameInput ? 1 : 0,
                        transform: showNameInput ? 'translateY(0)' : 'translateY(-20px)'
                    }}
                >
                    Enter your name
                </h3>
                
                <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="Your name"
                    className="w-[300px] px-4 py-3 rounded-lg border-2 border-gray-300 focus:border-[#4F39F6] focus:outline-none transition-all duration-200"
                    style={{
                        opacity: showNameInput ? 1 : 0,
                        transform: showNameInput ? 'translateY(0)' : 'translateY(-20px)',
                        transitionDelay: showNameInput ? '100ms' : '0ms'
                    }}
                    onKeyDown={(e) => {
                        if (e.key === 'Enter' && name.trim()) {
                            handleNext();
                        }
                    }}
                />
                
                <button 
                    onClick={handleNext}
                    disabled={!name.trim()}
                    className="rounded-md text-sm font-semibold px-4 py-2 bg-[#4F39F6] text-white min-w-[126px] cursor-pointer transition-all duration-300 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                    style={{
                        opacity: showNameInput ? 1 : 0,
                        transform: showNameInput ? 'translateY(0)' : 'translateY(-20px)',
                        transitionDelay: showNameInput ? '200ms' : '0ms'
                    }}
                >
                    Next
                </button>
            </div>

            {/* Avatar Selection Step */}
            <div 
                className={`absolute inset-0 flex flex-col items-center gap-12 py-12 transition-all duration-700 ease-in-out ${
                    showAvatarSelect 
                        ? 'opacity-100 translate-x-0 scale-100' 
                        : 'opacity-0 translate-x-[100px] scale-95 pointer-events-none'
                }`}
            >
                {/* Back Button */}
                <button
                    onClick={handleBack}
                    className="absolute left-0 top-0 mt-12 ml-8 w-10 h-10 flex items-center justify-center cursor-pointer transition-all duration-200 active:scale-95"
                    style={{
                        opacity: showAvatarSelect ? 1 : 0,
                        transitionDelay: showAvatarSelect ? '0ms' : '0ms'
                    }}
                >
                    <svg 
                        width="38" 
                        height="38" 
                        viewBox="0 0 28 28" 
                        fill="none" 
                        xmlns="http://www.w3.org/2000/svg"
                        className="transition-all duration-200 hover:opacity-70"
                    >
                        <path 
                            d="M16 8L10 14L16 20" 
                            stroke="#fff" 
                            strokeWidth="2.5" 
                            strokeLinecap="round" 
                            strokeLinejoin="round"
                        />
                    </svg>
                </button>

                <h3 
                    className="text-3xl font-semibold transition-all duration-700"
                    style={{
                        opacity: showAvatarSelect ? 1 : 0,
                        transform: showAvatarSelect ? 'translateY(0)' : 'translateY(-20px)'
                    }}
                >
                    Choose your avatar
                </h3>
                
                <div
                    style={{
                        opacity: showAvatarSelect ? 1 : 0,
                        transform: showAvatarSelect ? 'translateY(0) scale(1)' : 'translateY(-20px) scale(0.95)',
                        transitionDelay: showAvatarSelect ? '100ms' : '0ms'
                    }}
                >
                    <MemAvatarSelect onSelectAvatar={handleSelectAvatar} />
                </div>
                
                <button 
                    className="rounded-md text-sm font-semibold px-4 py-2 bg-[#4F39F6] text-white min-w-[126px] cursor-pointer transition-all duration-300 active:scale-95"
                    style={{
                        opacity: showAvatarSelect ? 1 : 0,
                        transform: showAvatarSelect ? 'translateY(0)' : 'translateY(-20px)',
                        transitionDelay: showAvatarSelect ? '200ms' : '0ms'
                    }}
                >
                    To play
                </button>
            </div>
        </div>
    )
};
