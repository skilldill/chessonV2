...
export function CreateTournamentScreen() {
  ...
  return (
    <div className="relative w-full min-h-full overflow-y-auto px-4 py-20">
      ...
      <main className="mx-auto flex w-full max-w-[390px] flex-col gap-5">
        <h1 className="text-white text-2xl font-bold">{t("tournament.newTitle")}</h1>
        ...
        <button
          ...
          className="btn-client h-12 rounded-lg bg-[#4f39f6] px-4 font-semibold text-white transition-opacity disabled:opacity-50"
        >
          {t("tournament.create")}
        </button>
...
export function TournamentRoomScreen() {
  ...
  const handleFinishTournament = async () => {
    ...
    if (data?.success) setShowFinishConfirm(false);
  };
  ...
  const getParticipantStatus = (item: TournamentParticipant) => {
    ...
    const currentMatch = currentRound?.matches.find((match) => match.playerAId === item.id || match.playerBId === item.id);
    if (currentMatch?.status === "active") return { label: t("tournament.status.playing"), className: "px-3 py-2 text-yellow-300" };
    ...
    <div className="relative w-full min-h-full overflow-y-auto px-4 py-20">
      ...
      <main className="mx-auto flex w-full max-w-[430px] flex-col gap-4">
        ...
        {tournament?.status === "scheduled" && tournament.startAt && (
          <div className="rounded-xl border border-[#555ab9b3] bg-[#4f39f633] p-4">
        ...
        {isLateParticipantWaiting && (
          <div className="rounded-xl border border-[#555ab9b3] bg-[#4f39f633] p-4 text-sm text-white/85">
            {t("tournament.lateJoinHint")}
          </div>
        )}

        {tournament?.waitingForPlayers && (
          <div className="rounded-xl border border-[#555ab9b3] bg-[#4f39f633] p-4 text-sm text-white/80">
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
            {!currentUserId && (
              <input
                ...
                className="mb-3 h-12 w-full rounded-xl border border-white/15 bg-black/20 px-3 text-white outline-none"
            ...
            <button
              ...
              onClick={handleJoin}
              className="btn-client h-12 w-full rounded-xl bg-[#4f39f6] font-semibold text-white disabled:opacity-50"
            >
        ...
        {!isViewOnly && participant && tournament?.status !== "finished" && participantInTournament?.active && (
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            {canOpenGame && <div className="mt-2 text-sm text-[#4f39f6]">{t("tournament.gameOpening")}</div>}
            <button
              type="button"
              disabled={isBusy}
              onClick={() => setShowLeaveConfirm(true)}
              className="btn-client btn-client-preset mt-4 h-11 w-full rounded-xl border border-white/15 text-sm text-white"
            >
              {t("tournament.leave")}
            </button>
          </section>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="flex flex-col gap-2">
              {(tournament.status === "setup" || (tournament.status === "scheduled" && tournament.rounds.length === 0)) && (
                playerCount < 3 ? (
                  <div className="rounded-xl border border-[#555ab9b3] bg-[#4f39f633] px-3 py-3 text-center text-sm text-white/85">
                    {t("tournament.minPlayersHint")}
                  </div>
                ) : (
                  <>
                    <button type="button" disabled={isBusy} onClick={() => callAction("/start", { delaySeconds: 0 })} className="btn-client h-11 rounded-xl bg-[#4f39f6] text-sm font-semibold text-white disabled:opacity-50">{t("tournament.startNow")}</button>
                    <button type="button" disabled={isBusy} onClick={() => callAction("/start", { delaySeconds: 60 })} className="btn-client btn-client-preset h-11 rounded-xl bg-white/10 text-sm text-white">{t("tournament.startInOneMinute")}</button>
                    <button type="button" disabled={isBusy} onClick={() => callAction("/start", { delaySeconds: 300 })} className="btn-client btn-client-preset h-11 rounded-xl bg-white/10 text-sm text-white">{t("tournament.startInFiveMinutes")}</button>
                  </>
                )
              )}
              {tournament.status === "running" && (
                <>
                  ...
                  {tournament.finishAfterCurrentRound ? (
                    ...
                    <button type="button" disabled={isBusy} onClick={() => setShowFinishConfirm(true)} className="btn-client h-11 rounded-xl border border-[#555ab9b3] bg-[#4f39f633] text-sm text-white/85 disabled:opacity-50">
        ...
        {error && <div className="rounded-xl border border-red-500/50 bg-red-500/15 p-3 text-sm text-red-200">{error}</div>}

        {!isViewOnly && isCreator && tournament?.tieBreakAvailable && (
          <section className="grid gap-3 rounded-2xl border border-[#555ab9b3] bg-[#4f39f633] p-4">
            <div>
            ...
            <button type="button" disabled={isBusy} onClick={() => callAction("/create-tie-break")} className="btn-client h-11 rounded-xl bg-[#4f39f6] text-sm font-semibold text-white disabled:opacity-50">
              {t("tournament.createTieBreak")}
            </button>
            <button type="button" disabled={isBusy} onClick={() => callAction("/decline-tie-break")} className="btn-client btn-client-preset h-11 rounded-xl bg-white/10 text-sm text-white disabled:opacity-50">
        ...
        <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
          <h2 className="mb-3 text-lg font-semibold text-white">{t("tournament.players")}</h2>
                ...
                {sortedParticipants.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {tournament?.status !== "finished" && <td className={playerStatus.className}>{playerStatus.label}</td>}
        ...
        {tournament?.status === "finished" ? (
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <h2 className="mb-3 text-lg font-semibold text-white">{t("tournament.allRounds")}</h2>
      ...
      {!isViewOnly && showLeaveConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          ...
          <div className="relative w-full max-w-[390px] rounded-2xl border border-white/15 bg-[#121217] p-6 shadow-2xl">
            ...
            <p className="text-white/60 text-sm text-center mt-3">{t("tournament.leaveConfirmText")}</p>
            <div className="mt-5 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setShowLeaveConfirm(false)} className="btn-client btn-client-preset rounded-xl px-4 py-3 text-white font-semibold">
                {t("tournament.stay")}
              </button>
              <button type="button" disabled={isBusy} onClick={handleLeave} className="btn-client rounded-xl px-4 py-3 bg-red-500/80 text-white font-semibold disabled:opacity-50">
      ...
      {!isViewOnly && showFinishConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          ...
          <div className="relative w-full max-w-[390px] rounded-2xl border border-white/15 bg-[#121217] p-6 shadow-2xl">
            ...
            <p className="text-white/60 text-sm text-center mt-3">{t("tournament.finishConfirmText")}</p>
            <div className="mt-5 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setShowFinishConfirm(false)} className="btn-client btn-client-preset rounded-xl px-4 py-3 text-white font-semibold">
                {t("common.cancel")}
              </button>
              <button type="button" disabled={isBusy} onClick={handleFinishTournament} className="btn-client rounded-xl px-4 py-3 bg-[#4f39f6] text-white font-semibold disabled:opacity-50">
