...
import type { BotDifficulty } from "../../components/BotDifficultyModal/BotDifficultyModal";
...
type BotPlayerColor = "white" | "black";
...
export function GameAnalysisScreen() {
  ...
  const boardSize = Math.max(
    36,
    Math.floor(Math.min((viewport.width - 32) / 8, (viewport.height - 156) / 8, 56)),
  );
  ...
  const currentPly = selectedPly ?? 0;
  const canGoPrevious = currentPly > 0;
  const canGoNext = currentPly < maxPly;
  const goToPreviousMove = () => setSelectedPly((current) => Math.max(0, (current ?? 0) - 1));
  const goToNextMove = () => setSelectedPly((current) => Math.min(maxPly, (current ?? 0) + 1));
      ...
      <div className="mx-auto flex w-full max-w-[560px] flex-col gap-4">
        <section className="rounded-lg border border-white/10 bg-white/[0.04] p-3">
          ...
          <button
            ...
            className="mt-3 w-full rounded-md border border-[#4F39F6]/40 bg-[#4F39F6]/20 px-3 py-3 text-sm font-semibold text-white transition active:scale-[0.98]"
          >
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button
              ...
              className="rounded-md border border-white/10 bg-white/5 px-3 py-3 text-sm font-semibold text-white/80 transition active:scale-[0.98] disabled:opacity-35"
              aria-label={t("analysis.previousMove")}
            >
              ‹ {t("analysis.previousMove")}
            </button>
            <button
              type="button"
  ...
  return (
    <main className="min-h-screen bg-back-primary text-white">
      <div className="sticky left-0 right-0 top-0 z-30 border-b border-white/10 bg-black/25 backdrop-blur-md">
        <div className="relative mx-auto flex h-14 w-full max-w-[560px] items-center justify-center px-3">
          <button
            type="button"
            onClick={onGoHome}
            className="absolute left-2 flex h-9 items-center gap-1 rounded-md px-2 text-sm font-semibold text-white/80 transition active:scale-[0.98]"
          >
            <span className="h-2.5 w-2.5 rotate-45 border-b-2 border-l-2 border-current" aria-hidden="true" />
            <span>{t("analysis.goHome")}</span>
          </button>
          ...
          <button
            type="button"
            onClick={onShareGame}
            className="absolute right-2 flex h-9 w-9 items-center justify-center rounded-md border border-white/15 bg-white/5 text-white/80 transition active:scale-[0.98]"
            aria-label={isShareCopied ? t("analysis.shareCopied") : t("analysis.shareGame")}
            title={isShareCopied ? t("analysis.shareCopied") : t("analysis.shareGame")}
          >
            <LinkIcon />
          </button>
        </div>
...
function CreateFromPositionModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/70 p-3 backdrop-blur-sm">
      <div className="mx-auto my-6 w-full max-w-[390px] rounded-[24px] border border-white/15 bg-[#121217] p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            onClick={onClose}
            disabled={isCreating}
            className="rounded-md px-2 py-1 text-sm font-semibold text-white/55 transition active:scale-[0.98] disabled:opacity-50"
          >
            {t("common.close")}
          </button>
        </div>
        ...
        {fen && (
          <div className="mt-4 rounded-md border border-white/10 bg-black/20 p-3">
            <div className="text-sm font-semibold text-white/45">{t("bot.fenLabel")}</div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button
            ...
            disabled={isCreating}
            className={`rounded-md border px-3 py-3 text-sm font-semibold transition active:scale-[0.98] disabled:opacity-50 ${
              mode === "bot" ? "border-[#4F39F6] bg-[#4F39F6]/20 text-white" : "border-white/10 bg-white/5 text-white/65"
            }`}
          >
          ...
          <button
            ...
            disabled={isCreating}
            className={`rounded-md border px-3 py-3 text-sm font-semibold transition active:scale-[0.98] disabled:opacity-50 ${
              mode === "friend" ? "border-[#4F39F6] bg-[#4F39F6]/20 text-white" : "border-white/10 bg-white/5 text-white/65"
            }`}
          >
          ...
          <div className="mt-4 grid gap-2">
            {levels.map((level) => (
              <button
                ...
                disabled={isCreating}
                className={`rounded-md border px-3 py-3 text-left transition active:scale-[0.98] disabled:opacity-50 ${
              ...
              >
                ...
                <div className="mt-1 text-sm text-white/45">{level.subtitle}</div>
              </button>
            ))}
            <div className="mt-2">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2">
          <button
            ...
            onClick={onClose}
            disabled={isCreating}
            className="rounded-md border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white/70 transition active:scale-[0.98] disabled:opacity-50"
          >
            {t("common.cancel")}
          </button>
          <button
...
function MovePairRow({
  ...
  return (
    <div className="grid grid-cols-[28px_minmax(0,1fr)_minmax(0,1fr)] gap-2 rounded-md border border-white/10 bg-black/15 p-2">
  ...
  return (
    <button
      ...
      className={`min-w-0 rounded-md border p-2 text-left transition flex flex-col justify-start ${
        selected ? "border-[#4F39F6] bg-[#4F39F6]/20" : "border-transparent bg-white/[0.03]"
      }`}
    >
