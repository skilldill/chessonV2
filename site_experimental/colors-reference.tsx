export default function ColorsReference() {
  ...
  const customColors = {
    accent: "#5757EB", // Your custom accent color
    background: "#000000",
    foreground: "#ffffff",
  }
  ...
  return (
    <div className="min-h-screen bg-white p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-bold text-gray-900 mb-8">Tailwind CSS Color Palette</h1>
        ...
        <div className="space-y-8">
          {Object.entries(colors).map(([colorName, shades]) => (
            <div key={colorName}>
              <h3 className="text-xl font-semibold text-gray-800 mb-3 capitalize">{colorName}</h3>
              <div className="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-11 gap-2">
                {Object.entries(shades).map(([shade, hex]) => (
                  <div key={shade} className="text-center">
                    <div
                      className="w-full h-16 rounded-lg border border-gray-300 shadow-sm mb-2"
        ...
        <div className="mt-12 p-6 bg-gray-50 rounded-lg">
          ...
          <div className="space-y-2 text-sm font-mono text-gray-600">
            <div>
              Background: <span className="text-blue-600">bg-blue-500</span>
            </div>
            <div>
              Text: <span className="text-blue-600">text-red-600</span>
            </div>
            <div>
              Border: <span className="text-blue-600">border-green-400</span>
            </div>
            <div>
              Custom: <span className="text-blue-600">bg-[#5757EB]</span> (your accent color)
            </div>
          </div>
        </div>
      </div>
